...
- **列标题栏**：可选显示「名称 / 说明」表头；中间分隔条可拖拽调整列宽，双击恢复自动测量；列宽持久化到配置。
- **贴边行为**：`SystemSnapDisabler` 禁用 Win+方向键半边吸附、标题栏双击最大化等系统吸附，避免与自定义边缘停靠冲突。
- **窗口**：最小尺寸统一为 280×200 DIP；关闭时保存窗口位置与列宽。
- **设置**：新增「显示列表列标题栏」开关；列表字号 / 界面字号 / 窗口记忆等与 0.1.12 行为一并纳入本包。

| 包 | 版本 |
| --- | --- |
| cornerstone-bridge | 0.1.14 |
| cornerstone-web | 0.1.14 |
| cornerstone-cli | 0.1.14 |
